...
/**
 * Updates the main date display on the home screen.
 */
function updateMainDateDisplay() {
     if (elements.mainDateDisplay) {
         const now = new Date();
         try {
             // Example: "Today is Wednesday, 3 May 2025" (adjust locale/options as needed)
             const options = { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' };
             const formattedDate = now.toLocaleDateString(undefined, options); // Use browser's default locale
             elements.mainDateDisplay.textContent = `Today is ${formattedDate}`;
         } catch (e) {
             console.error("Error formatting main date display:", e);
             elements.mainDateDisplay.textContent = "Error loading date";
         }
     }
}
...
function createTaskListItem(task, viewType) {
    ...
    if (viewType === 'today' || viewType === 'dayView' || viewType === 'upcoming' || viewType === 'alert') {
         detailsHtml = `<p class="task-details"><i class="far fa-calendar-alt"></i> ${dateString} <i class="far fa-clock ml-2"></i> ${timeString} ${snoozeStatus}</p>`; // Append snoozeStatus here
...
function checkShowReturnToToday() {
    ...
    // Show the button only if we are NOT viewing the current month/year
    elements.returnToTodayBtn?.classList.toggle('hidden', isCurrentMonthAndYear);
}
...
async function showDayView(day) {
    ...
    // Update Day View Title to show the day name (e.g., "Wednesday")
    ...
    // Hide Month calendar elements and Month/Year navigation
    ...
    // Update main calendar title area (optional - maybe hide it instead?)
    // if(elements.calendarTitle) elements.calendarTitle.textContent = 'Day View'; // Or hide the whole header?
...
function showCalendarView() {
    ...
    // Reset main calendar title area (optional)
    // if(elements.calendarTitle) elements.calendarTitle.textContent = 'Calendar';
...
function addTaskForSelectedDay() {
    ...
    let defaultHour = defaultTime.getHours(); // 9
    let defaultAmPm = 'AM';
    ...
    // Switch to the Add Task section
    showSection('addTask'); // *** FIX: Use 'addTask' not 'add-task' ***
}
...
async function handleFreeUpTaskAction(event) {
     ...
     // Add snooze handling here if the snooze button is kept in the modal's task items
          ...
          const snoozeDuration = button.dataset.snooze;
           ...
           optionsDiv = taskItem.querySelector('.snooze-options');
